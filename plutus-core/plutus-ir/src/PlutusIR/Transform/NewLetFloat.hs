          ...
          l@(Let _ r bs tIn) ->
            ...
            in
              ...
              then do
                  ...
                  (_,scope) <- ask
                  let freeVars = (case r of Rec ->  (S.\\ fromList (bs^..traversed.bindingIds)); NonRec -> id) $ calcFreeVars bs
...
calcFreeVars :: (Ord name, Ord tyname, PLC.HasUnique tyname PLC.TypeUnique, PLC.HasUnique name PLC.TermUnique) => NE.NonEmpty (Binding tyname name uni fun ann)
             -> S.Set PLC.Unique
calcFreeVars = foldMap1 $ \b ->
